...
namespace BitCode\BITCF7ZC\Integration\ZohoCRM;

use WP_Error;
use BitCode\BITCF7ZC\Core\Util\HttpHelper;
use BitCode\BITCF7ZC\Integration\ZohoCRM\TagApiHelper;
use BitCode\BITCF7ZC\Integration\ZohoCRM\MetaDataApiHelper;
use BitCode\BITCF7ZC\Integration\ZohoCRM\RecordApiHelper;
use BitCode\BITCF7ZC\Integration\IntegrationHandler;
use BitCode\BITCF7ZC\Core\Util\IpTool;
use BitCode\BITCF7ZC\Admin\Log\Handler as Log;
...
final class ZohoCRMHandler
{
    ...
    {
        \add_action('wp_ajax_bitcf7zc_zcrm_generate_token', array(__CLASS__, 'generateTokens'));
        \add_action('wp_ajax_bitcf7zc_zcrm_refresh_modules', array(__CLASS__, 'refreshModulesAjaxHelper'));
        \add_action('wp_ajax_bitcf7zc_zcrm_refresh_layouts', array(__CLASS__, 'refreshLayoutsAjaxHelper'));
    }
    ...
    {
        if (isset($_REQUEST['_ajax_nonce']) && wp_verify_nonce(sanitize_text_field($_REQUEST['_ajax_nonce']), 'bitcf7_nonce')) {
    ...
    {
        add_filter('bitcf7zc_addRelatedList', array(__CLASS__, 'addRelatedList'), 10, 6);
    }
            ...
            if (
                ...
                wp_send_json_error(
                    !empty($assignmentRulesResponse->status)
                        ...
                        $assignmentRulesResponse->message : (empty($assignmentRulesResponse) ? __('Assignment rules is empty', 'bitcf7zc') : 'Unknown'),
            ...
            if (
                ...
                wp_send_json_error(
                    !empty($relatedListResponse->status)
                        ...
                        $relatedListResponse->message : (empty($relatedListResponse) ? __('RelatedList is empty', 'bitcf7zc') : 'Unknown'),
    ...
    public function execute(IntegrationHandler $integrationHandler, $integrationData, $fieldValues)
    {
        ...
        if (
            ...
            $error = new WP_Error('REQ_FIELD_EMPTY', __('module, layout, fields are required for zoho crm api', 'bitcf7zc'));
        ...
        if (
            ...
            $zcrmApiResponse = apply_filters('bitcf7zc_addRelatedList', $zcrmApiResponse, $this->_formID, $entryID, $this->_integrationID, $logID, $fieldValues, $integrationDetails, $recordApiHelper);
        }
        return $zcrmApiResponse;
    }
}
