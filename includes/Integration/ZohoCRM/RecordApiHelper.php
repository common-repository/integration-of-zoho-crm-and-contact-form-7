...
use BitCode\BITCF7ZC\Integration\ZohoCRM\FilesApiHelper;
...
class RecordApiHelper
{
    ...
    public function executeRecordApi($formID, $integId, $defaultConf, $module, $layout, $fieldValues, $fieldMap, $actions, $required, $fileMap = [], $isRelated = false)
    {
        ...
        foreach ($fieldMap as $fieldKey => $fieldPair) {
            if (!empty($fieldPair->zohoFormField)) {
                ...
                if (empty($fieldData[$fieldPair->zohoFormField]) && \in_array($fieldPair->zohoFormField, $required)) {
                    $error = new WP_Error('REQ_FIELD_EMPTY', wp_sprintf(__('%s is required for zoho crm, %s module', 'bitcf7zc'), $fieldPair->zohoFormField, $module));
                ...
                if (!empty($fieldData[$fieldPair->zohoFormField])) {
                    ...
                    if ($currentLength > $requiredLength) {
                        $error = new WP_Error('REQ_FIELD_LENGTH_EXCEEDED', wp_sprintf(__('zoho crm field %s\'s maximum length is %s, Given %s', 'bitcf7zc'), $fieldPair->zohoFormField, $module));
        ...
        if (
            ...
            if (!empty($actions->tag_rec) && class_exists('BitCode\BITCF7ZC\Integration\ZohoCRM\TagApiHelper')) {
                $tags = '';
